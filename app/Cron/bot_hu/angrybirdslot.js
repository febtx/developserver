...
let spin = function(io, user){
	...
	let nohu      = false;
	// tạo kết quả
	HU.findOne({game:'AngryBirdSlot', type:bet, red:true}, {}, function(err2, dataHu){
				...
				if(!nohu && line_win.win == 10) {
					...
					if (line_win.type === 5) {
						...
						if (!nohu) {
							...
							bet_win += okHu;
							HU.updateOne({game:'AngryBirdSlot', type:bet, red:true}, {$set:{name:'', bet:dataHu.min}}).exec();
						}else{
...
/* 			if (!nohu && bet_win >= tongCuoc*2.24) {
			...
			HU.updateOne({game:'AngryBirdSlot', type:bet, red:true}, {$inc:{bet:addQuy}}).exec();
...
module.exports = function(io, listBot){
	if(listBot == null) {
		//console.log("listBot=null game AngryBirdSlot")
