...
function gameBonus(client, bet){
	...
	.then(result => {
		client.LanKwaiFong.bonus = result;
	});
}
// lankwaifong
module.exports = function(client, data){
	if (!!data && !!data.cuoc && Array.isArray(data.line)) {
		...
		if (!(bet == 100 || bet == 1000 || bet == 10000) || line.length < 1) {
			client.red({LanKwaiFong:{status:0}, notice:{text:'DỮ LIỆU KHÔNG ĐÚNG...', title:'THẤT BẠI'}});
		}else{
			client.LanKwaiFong = void 0 === client.LanKwaiFong ? {id:'', bet:bet, bonus:null, bonusX:0, bonusL:0, bonusWin:0, free:0} :client.LanKwaiFong;
			client.LanKwaiFong.bet = bet;
			let tongCuoc = bet*line.length;
			UserInfo.findOne({id:client.UID}, 'red name', function(err, user){
				if (!user || client.LanKwaiFong.free === 0 && user.red < tongCuoc) {
					client.red({LanKwaiFong:{status:0, notice:'Bạn không đủ R để quay.!!'}});
				}else{
					let config = Helpers.getConfig('lankwaifong');
					...
					let isBigWin = false;
					// tạo kết quả
					HU.findOne({game:'lankwaifong', type:bet}, {}, function(err2, dataHu){
						if (dataHu === null){
							return void 0;
						}
						let uInfo      = {hu:0};
						...
						.then(result => {
								...
								if (line_win.win == 6) {
									...
									if (line_win.type === 5) {
										...
										if (!nohu) {
											...
											bet_win += okHu;
											HU.updateOne({game:'lankwaifong', type:bet}, {$set:{name:'', bet:dataHu.min}}).exec();
							...
							let tien = 0;
							if (client.LanKwaiFong.free > 0) {
								tien = bet_win;
								client.LanKwaiFong.free -= 1;
							}else{
							...
							if (free > 0) {
								client.LanKwaiFong.free += free;
							}
							if (!!bonusX) {
								client.LanKwaiFong.bonusX += bonusX;
								client.LanKwaiFong.bonusL = 10;
							...
							client.red({LanKwaiFong:{status:1, cel:[cel1, cel2, cel3, cel4, cel5], line_win:result, win:bet_win, free:client.LanKwaiFong.free, isFree:isFree, isBonus:!!client.LanKwaiFong.bonusX, isNoHu:nohu, isBigWin:isBigWin}, user:{red:user.red-tongCuoc}});
							LanKwaiFong_red.create({'name':client.profile.name, 'type':type, 'win':bet_win, 'bet':bet, 'kq':result.length, 'line':line.length, 'time':new Date()}, function (err4, small) {
								client.LanKwaiFong.id = small._id.toString();
							});
							...
							HU.updateOne({game:'lankwaifong', type:bet}, {$inc:huUpdate}).exec();
							UserInfo.updateOne({id:client.UID},{$inc:uInfo}).exec();
							LanKwaiFong_users.updateOne({'uid':client.UID}, {$set:{time:new Date().getTime(), select:bet}, $inc:mini_users}).exec();
