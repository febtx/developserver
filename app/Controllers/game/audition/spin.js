...
module.exports = function(client, data){
	if (!!data && !!data.cuoc && Array.isArray(data.line)) {
		...
		if (!(bet == 100 || bet == 1000 || bet == 10000) || line.length < 1) {
			client.red({Audition:{status:0}, notice:{text:'DỮ LIỆU KHÔNG ĐÚNG...', title:'THẤT BẠI'}});
		}else{
			client.Audition = void 0 === client.Audition ? {id:'', bet:bet, bonus:null, bonusX:0, bonusL:0, bonusWin:0, free:0} :client.Audition;
			client.Audition.bet = bet;
			let tongCuoc = bet*line.length;
			UserInfo.findOne({id:client.UID}, 'red name', function(err, user){
				if (!user || client.Audition.free === 0 && user.red < tongCuoc) {
					client.red({Audition:{status:0, notice:'Bạn không đủ R để quay.!!'}});
				}else{
					let config = Helpers.getConfig('audition');
					...
					let isBigWin = false;
					// tạo kết quả
					HU.findOne({game:'audition', type:bet}, {}, function(err2, dataHu){
						if (dataHu === null){
							return void 0;
						}
						let uInfo      = {hu:0};
						...
						.then(result => {
								...
								if (line_win.win == 6) {
									...
									if (line_win.type === 5) {
										...
										if (!nohu) {
											...
											bet_win += okHu;
											HU.updateOne({game:'audition', type:bet}, {$set:{name:'', bet:dataHu.min}}).exec();
											client.redT.sendInHome({pushnohu:{title:'Audition', name:client.profile.name, bet:okHu}});
										}else{
							...
							let tien = 0;
							if (client.Audition.free > 0) {
								tien = bet_win;
								client.Audition.free -= 1;
							}else{
							...
							if (!nohu && bet_win >= tongCuoc*2.24) {
								isBigWin = true;
								//type = 1;
								bet_win >= 10000 && client.redT.sendInHome({news:{t:{game:'Audition', users:client.profile.name, bet:bet_win, status:2}}});
							}
							if (free > 0) {
								client.Audition.free += free;
							}
							if (!!bonusX) {
								client.Audition.bonusX += bonusX;
								client.Audition.bonusL = 10;
							...
							client.red({Audition:{status:1, cel:[cel1, cel2, cel3, cel4, cel5], line_win:result, win:bet_win, free:client.Audition.free, isFree:isFree, isBonus:!!client.Audition.bonusX, isNoHu:nohu, isBigWin:isBigWin}, user:{red:user.red-tongCuoc}});
							Audition_red.create({'name':client.profile.name, 'type':type, 'win':bet_win, 'bet':bet, 'kq':result.length, 'line':line.length, 'time':new Date()}, function (err4, small) {
								client.Audition.id = small._id.toString();
							});
							...
							HU.updateOne({game:'audition', type:bet}, {$inc:huUpdate}).exec();
							UserInfo.updateOne({id:client.UID},{$inc:uInfo}).exec();
							Audition_users.updateOne({'uid':client.UID}, {$set:{time:new Date().getTime(), select:bet}, $inc:mini_users}).exec();
