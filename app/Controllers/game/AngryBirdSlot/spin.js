...
let AngryBirdSlot_percent = require('../../../Models/AngryBirdSlot/AngryBirdSlot_percent');
...
function gameBonus(client, bet){
	...
	.then(result => {
		client.AngryBirdSlot.bonus = result;
	});
}

function loadCached(bet){
		randomPer = AngryBirdSlot_percent.findOne({type:bet}).exec();
		if(!!randomPer){
...
module.exports = function(client, data){
	if (!!data && !!data.cuoc && Array.isArray(data.line)) {
		...
		if (!(bet == 100 || bet == 1000 || bet == 10000) || line.length < 1) {
			client.red({AngryBirdSlot:{status:0}, notice:{text:'DỮ LIỆU KHÔNG ĐÚNG...', title:'THẤT BẠI'}});
		}else{
			client.AngryBirdSlot = void 0 === client.AngryBirdSlot ? {id:'', bet:bet, bonus:null, bonusL:0, bonusWin:0, free:0} :client.AngryBirdSlot;
			client.AngryBirdSlot.bet = bet;
			let tongCuoc = bet*line.length;
			UserInfo.findOne({id:client.UID}, 'red name', function(err, user){
				if (!user || client.AngryBirdSlot.free === 0 && user.red < tongCuoc) {
					client.red({AngryBirdSlot:{status:0, notice:'Bạn không đủ Gold để quay.!!'}});
				}else{
					let config = Helpers.getConfig('AngryBirdSlot');
					let phe = red = 2;    // Phế
					...
					// tạo kết quả
					HU.findOne({game:'AngryBirdSlot', type:bet}, {}, function(err2, dataHu){
						if (dataHu === null){
							return void 0;
						}
						let uInfo      = {hu:0};
						...
						let celSS = null;
						if(line.length<15){
							console.log("AngryBirdSlot, Số dòng chọn: "+line.length)
							celSS = [
						...
						if (bonusScreen >= 5) {
							// Bonus 5
							client.AngryBirdSlot.bonusL = 5;
							gameBonus(client, bet);
						}else if (bonusScreen == 4){
							// Bonus 4
							client.AngryBirdSlot.bonusL = 4;
							gameBonus(client, bet);
						}else if (bonusScreen == 3){
							// Bonus 3
							client.AngryBirdSlot.bonusL = 3;
							gameBonus(client, bet);
						}
						...
						.then(result => {
								...
								if(!nohu && line_win.win == 10) {
									...
									if (line_win.type === 5) {
										...
										if (!nohu) {
											...
											bet_win += okHu;
											HU.updateOne({game:'AngryBirdSlot', type:bet}, {$set:{name:'', bet:dataHu.min}}).exec();
											red && client.redT.sendInHome({pushnohu:{title:'AngryBirdSlot', name:client.profile.name, bet:okHu}});
										}else{
							...
							let tien = 0;
							if (client.AngryBirdSlot.free > 0) {
								tien = bet_win;
								client.AngryBirdSlot.free -= 1;
							}else{
							...
							if (!nohu && bet_win >= tongCuoc*2.24) {
								...
								//type = 1;
								//if (red) {
									bet_win >= 10000 && client.redT.sendInHome({news:{t:{game:'AngryBirdSlot', users:client.profile.name, bet:bet_win, status:2}}});
								//}
							}
							if (free > 0) {
								client.AngryBirdSlot.free += free;
							}
							...
							let thuong = 0;
								...
								AngryBirdSlot_red.create({'name':client.profile.name, 'type':type, 'win':bet_win, 'bet':bet, 'kq':result.length, 'line':line.length, 'time':new Date()}, function (err4, small) {
									client.red({AngryBirdSlot:{status:1, cel:[cel1, cel2, cel3, cel4, cel5], line_win:result, win:bet_win, free:client.AngryBirdSlot.free, isFree:isFree, isBonus:client.AngryBirdSlot.bonusL, isNoHu:nohu, isBigWin:isBigWin, phien:small.id}, user:{red:user.red-tongCuoc}});
									client.AngryBirdSlot.id = small._id.toString();
								});
							...
							HU.updateOne({game:'AngryBirdSlot', type:bet}, {$inc:huUpdate}).exec();
							UserInfo.updateOne({id:client.UID},{$inc:uInfo}).exec();
							AngryBirdSlot_user.updateOne({'uid':client.UID}, {$set:{time:new Date()}, $inc:mini_users}).exec();
